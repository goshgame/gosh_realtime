...
public class OnlineCFJob {
    ...
    private static class PairCountRedisSink extends RichSinkFunction<PairScoreEvent> {
        ...
        private static final long BATCH_FLUSH_INTERVAL_MS = 1000;
        ...
        private void maybeFlushBatch(boolean force) {
            ...
            if (force || batchBuffer.size() >= BATCH_SIZE || (now - lastFlushTime) >= BATCH_FLUSH_INTERVAL_MS) {
                flushBatch();
                lastFlushTime = now;
            }
        }
    ...
    static class OnlineCFConfig implements Serializable {
        ...
        // Flink配置
        private static final int PARALLELISM = 0; // 0 表示使用 Flink 默认配置
