...
public class OnlineCFJob {
    ...
    public static void main(String[] args) throws Exception {
        ...
        pairWithDecayStream.addSink(new PairScoreRedisSink(config, pairRedisConfig))
                .name("redis-pair-sink");
    ...
    private static class PairScoreRedisSink extends RichSinkFunction<PairScoreEvent> {

        private final OnlineCFConfig config;
        private final RedisConfig redisConfig;
        private transient RedisHelper redisHelper;

        PairScoreRedisSink(OnlineCFConfig config, RedisConfig redisConfig) {
            this.config = config;
            this.redisConfig = redisConfig;
        }
        ...
        public void invoke(PairScoreEvent value, Context context) {
            ...
            String indexKey = config.getIndexKeyPrefix() + value.leftItemId;
            redisHelper.zadd(indexKey, value.score, String.valueOf(value.rightItemId));
            long overflow = redisHelper.zcard(indexKey) - config.getIndexLimit();
            if (overflow > 0) {
                redisHelper.zremRangeByRank(indexKey, 0, overflow - 1);
            }
