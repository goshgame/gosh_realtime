 ...
 * 用户实时负反馈标签队列维护任务
 */
public class UserFeatureRFNFTest {
    private static final Logger LOG = LoggerFactory.getLogger(UserFeatureRFNFTest.class);
                    ...
                    .map(new MapFunction<UserNegativeTagQueue, Tuple2<String, byte[]>>() {
                        ...
                        public Tuple2<String, byte[]> map(UserNegativeTagQueue queue) throws Exception {
                            ...
                            if (queue.authorIds != null) {
                                System.out.println("添加负反馈作者ID，数量: " + queue.authorIds.size());
                                for (Long authorId : queue.authorIds) {
                                    ...
                                    System.out.println("添加作者ID: " + authorId);
                                }
                            } else {
                                System.out.println("无负反馈作者ID");
                            }
    ...
    private static class NegativeTagQueueProcessor extends org.apache.flink.streaming.api.functions.windowing.ProcessWindowFunction<
        ...
        public void process(Long uid,
            ...
            System.out.println("读取现有标签和作者队列...");
            ...
            System.out.println("现有作者ID数量: " + existingAuthorIds.size() + ", 内容: " + existingAuthorIds);
            ...
            for (NegativeFeedbackEvent event : elements) {
                events.add(event);
                System.out.println("处理事件 - PostID: " + event.postId + ", AuthorID: " + event.authorId + ", 时间戳: " + event.timestamp);

                // 异步从 Redis 获取视频标签（使用默认标签集合）
                CompletableFuture<String> tagFuture = getPostTagFromRedis(event.postId);
                tagFutures.add(tagFuture);

                // 处理作者ID队列
                if (event.authorId > 0) {
                    boolean existed = existingAuthorIds.remove(event.authorId);
                    if (existed) {
                        System.out.println("作者ID已存在，先移除: " + event.authorId);
                    }
                    existingAuthorIds.add(event.authorId);
                    System.out.println("作者ID添加到队列: " + event.authorId);
                    trimAuthorQueue(existingAuthorIds);
                } else {
                    System.out.println("事件无有效作者ID (AuthorID: " + event.authorId + ")");
                }
            }
            ...
            if (!existingTags.isEmpty() || !existingAuthorIds.isEmpty()) {
                ...
                System.out.println("输出标签和作者队列 - UID: " + uid +
                        ", 最终标签数量: " + queue.tags.size() +
                        ", 标签: " + queue.tags +
                        ", 最终作者ID数量: " + queue.authorIds.size() +
                        ", 作者ID: " + queue.authorIds);

                out.collect(queue);
            } else {
                System.out.println("标签和作者队列均为空，无输出");
            }
            ...
            try {
                ...
                if (tuple != null && tuple.f1 != null && tuple.f1.length > 0) {
                    ...
                    try {
                        ...
                        System.out.println("获取到反馈作者ID数量: " + feedbackAuthorIds.size());
                        for (RecFeature.FeedbackAuthorId feedbackAuthorId : feedbackAuthorIds) {
                            ...
                            if (authorId > 0) {
                                ...
                                System.out.println("现有作者ID: " + authorId);
                            }
                        }
                    } catch (Exception e) {
                        System.err.println("获取作者ID列表失败: " + e.getMessage());
                ...
                System.err.println("读取现有标签和作者失败: " + e.getMessage());
        ...
        private void trimAuthorQueue(LinkedHashSet<Long> authorIds) {
            while (authorIds.size() > MAX_NEGATIVE_TAGS) {
                ...
                if (iterator.hasNext()) {
                    ...
                    iterator.remove();
                    System.out.println("作者队列超限，移除最旧作者ID: " + removedAuthorId);
                } else {
                    break;
                }
            }
