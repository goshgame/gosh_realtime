...
//            .name("Debug Sampling");
        ...
        DataStream<Tuple2<String, byte[]>> dataStream = aggregatedStream
                .map(new MapFunction<UserFeature24hAggregation, Tuple2<String, byte[]>>() {
                    ...
                    public Tuple2<String, byte[]> map(UserFeature24hAggregation agg) throws Exception {
                        ...
                        agg.viewer3sviewPostHis24h.forEach(builder::addViewer3SviewPostHis24H);
                        agg.viewer5sstandPostHis24h.forEach(builder::addViewer5SstandPostHis24H);
                        agg.viewerLikePostHis24h.forEach(builder::addViewerLikePostHis24H);
                        agg.viewerFollowPostHis24h.forEach(builder::addViewerFollowPostHis24H);
                        agg.viewerProfilePostHis24h.forEach(builder::addViewerProfilePostHis24H);
                        agg.viewerPosinterPostHis24h.forEach(builder::addViewerPosinterPostHis24H);

                        builder.addAllViewerLikeAuthorHis24H(agg.viewerLikeAuthorHis24h);
                        builder.addAllViewerFollowAuthorHis24H(agg.viewerFollowAuthorHis24h);
                        builder.addAllViewerProfileAuthorHis24H(agg.viewerProfileAuthorHis24h);
    ...
    public static class UserFeature24hAggregator implements AggregateFunction<UserFeatureEvent, UserFeatureAccumulator, UserFeature24hAggregation> {
        ...
        public UserFeature24hAggregation getResult(UserFeatureAccumulator accumulator) {
            ...
            // 24小时历史记录特征
            result.viewer3sviewPostHis24h = UserFeatureCommon.buildIdScoreList(accumulator.view3sPostDetails, 10);
            result.viewer5sstandPostHis24h = UserFeatureCommon.buildIdScoreList(accumulator.stand5sPostDetails, 10);
...
//            LOG.info("Generated user 24h feature aggregation for uid {}: {}", result.uid, result);
    ...
    public static class UserFeature24hAggregation {
        ...
        // 24小时历史记录特征
        public List<RecFeature.IdScore> viewer3sviewPostHis24h = Collections.emptyList();
        public List<RecFeature.IdScore> viewer5sstandPostHis24h = Collections.emptyList();
        public List<Long> viewerLikePostHis24h = Collections.emptyList();
        public List<Long> viewerFollowPostHis24h = Collections.emptyList();
        public List<Long> viewerProfilePostHis24h = Collections.emptyList();
        public List<Long> viewerPosinterPostHis24h = Collections.emptyList();

        // 作者相关特征 (24小时)
        public List<Integer> viewerLikeAuthorHis24h = Collections.emptyList();
        public List<Integer> viewerFollowAuthorHis24h = Collections.emptyList();
        public List<Integer> viewerProfileAuthorHis24h = Collections.emptyList();
